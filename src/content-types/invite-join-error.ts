 ...
 */
export function encodeInviteJoinError(error: InviteJoinError): Uint8Array {
 ...
 */
export function decodeInviteJoinError(data: Uint8Array): InviteJoinError {
  const json = new TextDecoder().decode(data);
  const parsed = JSON.parse(json);

  // Handle unknown error types for forward compatibility
  let errorType: InviteJoinErrorType;
 ...
 */
export function createConversationExpiredError(inviteTag: string): InviteJoinError {
  return {
 ...
 */
export function createGenericFailureError(inviteTag: string): InviteJoinError {
  return {
 ...
 */
export function getErrorMessage(error: InviteJoinError): string {
