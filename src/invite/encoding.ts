 ...
 */
export function encodeToSlug(data: Uint8Array): string {
 ...
 */
export function decodeFromSlug(slug: string): Uint8Array {
...
export function generateInviteURL(
  slug: string,
  baseURL: string = "https://popup.convos.org/v2"
): string {
 ...
 */
function insertSeparators(str: string, separator: string, every: number): string {
  if (str.length <= every) {
    return str;
  }
